...
// LikeAction 点赞操作 todo 也可以设计成定时任务
func (*VideoService) LikeAction(ctx context.Context, req *video_server.FavoriteActionRequest) (resp *video_server.FavoriteActionResponse, err error) {
	resp = new(video_server.FavoriteActionResponse)

	key := fmt.Sprintf("%s:%s", "user", "like_count")
	setKey := fmt.Sprintf("%s:%s:%s", "video", "like_video", strconv.FormatInt(req.VideoId, 10))
	likeKey := fmt.Sprintf("%s:%s:%s", "user", "like_list", strconv.FormatInt(req.UserId, 10))

	action := req.ActionType
	var like model.Like
	like.UserId = uint64(req.UserId)
	like.VideoId = uint64(req.VideoId)
	...
	if action == 1 {
		...
		// 操作favorite表
		tx := model.DB.Begin()
		err = model.GetFavoriteInstance().AddFavorite(tx, &like)
		if err != nil {
			//tx.Rollback()
			resp.StatusCode = exception.FavoriteErr
			resp.StatusMsg = exception.GetMsg(exception.FavoriteErr)
		...
		// 删除喜欢列表缓存
		err = cache.Redis.Del(cache.Ctx, likeKey).Err()
		if err != nil {
			return nil, fmt.Errorf("缓存错误：%v", err)
		}
		defer func() {
			go func() {
				...
				//再次删除缓存
				cache.Redis.Del(cache.Ctx, likeKey)
			}()
		}()
	...
	if action == 2 {
		...
		// 操作favorite表
		tx := model.DB.Begin()
		err = model.GetFavoriteInstance().DeleteFavorite(tx, &like)
		if err != nil {
			resp.StatusCode = exception.CancelFavoriteErr
			resp.StatusMsg = exception.GetMsg(exception.CancelFavoriteErr)
			return resp, err
